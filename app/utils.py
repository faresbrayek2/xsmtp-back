...
def get_seo_rank_da(url: str) -> int:
    """
    Retrieves the Domain Authority (DA) of the given URL.

    Args:
        url (str): The URL to get the DA for.

    Returns:
        int: The Domain Authority.
    """
    headers = {
    ...
    try:
        response = requests.post('https://checkpagerank.net/check-page-rank.php', headers=headers, data=data, timeout=10)
...
def get_seo_info(url: str) -> dict:
    """
    Retrieves the SEO information of the given URL.

    Args:
        url (str): The URL to get the SEO information for.

    Returns:
        dict: A dictionary containing the SEO information.
    """
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ar;q=0.8',
    ...
    metrics = {}
    try:
        response = requests.post('https://checkpagerank.net/check-page-rank.php', headers=headers, data=data, timeout=30)
        soup = BeautifulSoup(response.text, 'html.parser')
